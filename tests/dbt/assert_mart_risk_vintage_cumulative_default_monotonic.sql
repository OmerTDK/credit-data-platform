with ordered_mob as (
    select
        ...
        mart_risk_vintage_curve.cumulative_default_count,
        lag(mart_risk_vintage_curve.cumulative_default_count) over (
            partition by
                mart_risk_vintage_curve.origination_cohort_quarter,
                mart_risk_vintage_curve.product_type,
                mart_risk_vintage_curve.score_band
            order by mart_risk_vintage_curve.months_on_book
        ) as prev_cumulative_default_count
...
select
    ...
    ordered_mob.prev_cumulative_default_count,
    ordered_mob.cumulative_default_count
from ordered_mob
where
    ordered_mob.prev_cumulative_default_count is not null
    and ordered_mob.cumulative_default_count < ordered_mob.prev_cumulative_default_count
